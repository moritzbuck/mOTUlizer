...
ggplot(gtdb_dat[phylum %in% phyla_of_fig1], aes(x=mean_cogs, y=mean_variable, shape = db, col=class, size = nb_genomes))+
...
ggplot(gtdb_dat[phylum %in% phyla_of_fig1], aes(shape = db, x=mean_cogs, y=mean_variable/mean_cogs, col=family, size = nb_genomes))+
  geom_point(data = anon_dat, alpha = 0.2, col = '#dcdcdc')+
  geom_point( alpha = 0.7)+
#  geom_smooth(data = anon_dat, method = "lm", size = 1, alpha = 0.8, se=F)+
