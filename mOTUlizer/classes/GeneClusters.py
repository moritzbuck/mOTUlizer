...
class GeneClusters():
    ...
    def __init__(self, motus, gene2gene_clust, gene_clust2rep):
        self.genomes = [g for motu in motus for g in motu]


    @classmethod
    def compute_GeneClusters(cls, motus, name, precluster = False, threads = 4, method =  "mmseqsCluster"):
        name = name + method + "_"
        ...
        if all([faa.endswith(".gz") for faa in faas]):
            cat = "zcat "
        elif all([not faa.endswith(".gz") for faa in faas]) :
            cat = "cat "
        else :
            sys.exit("Please make my life easy, either all faas gziped or none gzipped ...")

        os.system(cat + " ".join([*faas]) + " > " + all_faas_file)
        ...
        GeneClusters(motu, gene2gene_clusters, gene_clusters2rep, gene2genome)
...
class GeneCluster():
    ...
    def __init__(self, genes, gene2genome, representative):
        self.genes = genes
