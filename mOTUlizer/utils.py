    ...
    for s in records:
        for f in s.features:
            ...
            if f.type not in {'gene', 'source'}:
                ...
                "source" : f"file={file}",
                "feature" : f.type,
                "start" : None if (hasattr(f,"location_operator") and f.location_operator == "join") else int(f.location.start)+1,
                "end" : None if (hasattr(f,"location_operator") and f.location_operator == "join") else  int(f.location.end),
                "score" : "",
