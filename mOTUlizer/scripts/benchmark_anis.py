...
class Genome(object):
     ...
     def real_ani(self, other):
         simi = lambda a,b : sum([xy[0] == xy[1] for i,xy in enumerate(zip(a,b)) if i> (self.length*self.var_fract)])/(len(a)*(1-self.var_
 fract))
...
test1 =[]
 ...
 #    genome = MetaBin(j)
     ...
     print(f"===== {genome.name} =====")
     for mut in [1 - l/100 for l in range(85, 100, 1)]:
         for var in [0,0.05,0.10,0.20]:
             ...
             out['avg_cont'] = g2.minhash_ani(g, method = "avg_containment")
             out['max_cont'] = g2.minhash_ani(g, method = "max_containment")
             out['jacc'] = g2.minhash_ani(g, method="jaccard")
             out['var_fract'] = g2.var_fract
             out['mut_rate'] = g2.mutation_rate
             out['phylum'] = json.loads(db.get_genome_info(t['genome'])['taxonomy'].replace('""','"'))['r207'].split(";c__")[0]
             out['family'] = json.loads(db.get_genome_info(t['genome'])['taxonomy'].replace('""','"'))['r207'].split(";s__")[0]
             test1 += [out]
